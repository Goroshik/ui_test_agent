...
export class TestGenerator {
  async generate(
    ...
    const sessionId = this._resolveSessionId(meta, sessionDir);
  ...
  private _resolveSessionId(meta: SessionMeta | null, sessionDir: string): string {
    return meta?.sessionId ?? sessionDir.split(/[\\/]/).pop() ?? 'unknown';
  }
  ...
  private _buildPromptHeader(ctx: PromptContext): string {
    const { sessionId, pagePath, scenarioName, scenario, steps, components, sections } = ctx;
...
`;
  ...
  private _buildLoginRule(isLoginPage: boolean): string {
    return isLoginPage
  ...
  private _buildPrompt(ctx: PromptContext): string {
    const header = this._buildPromptHeader(ctx);
    const loginRule = this._buildLoginRule(ctx.isLoginPage);
    return `${header}${PROMPT_GENERATION_RULES_HEAD}${loginRule}${PROMPT_STATIC_TAIL}`;
  }
